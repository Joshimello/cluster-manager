...
| Desired-state reconciliation | Versioned, authenticated, workstation-scoped state; Linux username validation; root-only provenance ledger; idempotent changes; unowned username collisions and missing/invalid state fail closed; revocation locks access but preserves homes and running work. | Never adopt a pre-existing account. Resolve collisions by choosing another platform username or deliberately removing/renaming the local account. |
