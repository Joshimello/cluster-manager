...
sudo install -o root -g root -m 0644 \
  node/deploy/cluster-node.service /etc/systemd/system/cluster-node.service
sudo systemctl daemon-reload
sudo systemctl enable --now cluster-node
sudo journalctl -u cluster-node -f
```
...
```bash
sudo systemctl stop cluster-node
sudo install -o root -g root -m 0755 cluster-node-linux-amd64 \
  /usr/local/sbin/cluster-node
sudo systemctl start cluster-node
...
2. On the workstation, stop and remove the service and binary:
   ...
   ```bash
   sudo systemctl disable --now cluster-node
   sudo rm /usr/local/sbin/cluster-node
   sudo rm /etc/systemd/system/cluster-node.service
   sudo systemctl daemon-reload
   ```
