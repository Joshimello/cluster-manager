  ...
  <main class="mx-auto grid w-full max-w-7xl gap-6 px-4 py-8 sm:px-6 lg:px-8">
    ...
    <Card.Root>
      ...
      <Card.Content class="grid gap-6 sm:grid-cols-2 lg:grid-cols-5 lg:items-end">
        ...
        <div class="grid gap-1">
          <span class="text-muted-foreground text-xs font-medium uppercase">Workstations</span
          ><strong>{data.assignments.length} assigned</strong>
        </div>
    ...
    {#if data.assignments.length > 0}
      <div class="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 class="text-xl font-semibold tracking-tight">Workstation monitoring</h2>
          <p class="text-muted-foreground text-sm">Current values and recent telemetry history.</p>
        </div>
      ...
      {#each data.assignments as assignment (assignment.id)}
          ...
          <Card.Root>
            ...
            <Card.Content class="grid gap-3">
              ...
              {:else if assignment.provisioningErrorCode === 'uid_collision'}
                <p class="text-destructive text-sm font-medium">
                  UID {data.user.posixUid} is already in use on this workstation. Ask its operator to
                  resolve the unrelated identity; no ownership, credentials, groups, or files were changed.
                </p>
              ...
              {:else if assignment.provisioningErrorCode === 'managed_identity_mismatch'}
                <p class="text-destructive text-sm font-medium">
                  This managed account has the wrong numeric identity and must be purged and
                  recreated by the workstation operator.
                </p>
              {/if}
            </Card.Content>
          </Card.Root>
        ...
        {#if assignment.inventory}
          <HostMonitor
            ...
            observedAt={assignment.inventoryObservedAt}
            telemetryState={assignment.connectionState}
            timeZone={data.user.timeZone ?? 'UTC'}
          />
        {/if}

        <section class="grid gap-3" aria-labelledby={`gpu-monitoring-heading-${assignment.id}`}>
          <div>
            <h2
              id={`gpu-monitoring-heading-${assignment.id}`}
